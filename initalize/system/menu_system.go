...
// Exist 判断资源是否存在
func (s MenuSystem) Exist(filter map[string]interface{}) bool {
...
// Count 获取资源总数
func (s MenuSystem) Count() (int64, error) {
	var count int64
...
// Add 创建资源
func (s MenuSystem) Add(menu *model.Menu) error {
	return common.DB.Create(menu).Error
}

// Update 更新资源
func (s MenuSystem) Update(menu *model.Menu) error {
	return common.DB.Model(&model.Menu{}).Where("id = ?", menu.ID).Updates(menu).Error
}

// Find 获取单个资源
func (s MenuSystem) Find(filter map[string]interface{}, data *model.Menu) error {
	return common.DB.Where(filter).First(&data).Error
}

// List 获取数据列表
func (s MenuSystem) List() (menus []*model.Menu, err error) {
	err = common.DB.Order("sort").Find(&menus).Error
	return menus, err
}

// List 获取数据列表
func (s MenuSystem) ListUserMenus(roleIds []uint) (menus []*model.Menu, err error) {
	err = common.DB.Where("id IN (select menu_id as id from role_menus where role_id IN (?))", roleIds).Order("sort").Find(&menus).Error
	return menus, err
}

// 批量删除资源
func (s MenuSystem) Delete(menuIds []uint) error {
...
func (s MenuSystem) GetUserMenusByUserId(userId uint) ([]*model.Menu, error) {
	// 获取用户
	var user model.User

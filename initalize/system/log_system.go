...
type OperationLogSystem struct{}

//var Logs []model.OperationLog //全局变量多个线程需要加锁，所以每个线程自己维护一个
//处理OperationLogChan将日志记录到数据库
func (s OperationLogSystem) SaveOperationLogChannel(olc <-chan *model.OperationLog) {
	// 只会在线程开启的时候执行一次
...
// List 获取数据列表
func (s OperationLogSystem) List(req *request.OperationLogListReq) ([]*model.OperationLog, error) {
...
// Count 获取数据总数
func (s OperationLogSystem) Count() (count int64, err error) {
	err = common.DB.Model(&model.OperationLog{}).Count(&count).Error
	return count, err
}

// 获取单个用户
func (s OperationLogSystem) Find(filter map[string]interface{}, data *model.OperationLog) error {
	return common.DB.Where(filter).First(&data).Error
}

// Exist 判断资源是否存在
func (s OperationLogSystem) Exist(filter map[string]interface{}) bool {
...
// Delete 删除资源
func (s OperationLogSystem) Delete(operationLogIds []uint) error {

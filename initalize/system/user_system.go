...
// Add 添加资源
func (s UserSystem) Add(user *model.User) error {
...
// List 获取数据列表
func (s UserSystem) List(req *request.UserListReq) ([]*model.User, error) {
	var list []*model.User
...
func (s UserSystem) ListCount(req *request.UserListReq) (int64, error) {
	var count int64
	db := common.DB.Model(&model.User{}).Order("id DESC")
...
// List 获取数据列表
func (s UserSystem) ListAll() (list []*model.User, err error) {
...
// Count 获取数据总数
func (s UserSystem) Count() (int64, error) {
	var count int64
...
// Exist 判断资源是否存在
func (s UserSystem) Exist(filter map[string]interface{}) bool {
...
func (s UserSystem) FindTheSameUserName(username string, data *model.User) error {
	return common.DB.Where("username REGEXP ? ", fmt.Sprintf("^%s[0-9]{0,3}$", username)).Order("username desc").First(&data).Error
}

// Find 获取单个资源
func (s UserSystem) Find(filter map[string]interface{}, data *model.User) error {
	return common.DB.Where(filter).Preload("Roles").First(&data).Error
}

// Update 更新资源
func (s UserSystem) Update(user *model.User) error {
...
func (s UserSystem) GetUserMinRoleSortsByIds(ids []uint) ([]int, error) {
	// 根据用户ID获取用户信息
...
func (s UserSystem) GetCurrentUserMinRoleSort(c *gin.Context) (uint, model.User, error) {
	// 获取当前用户
...
// Delete 批量删除
func (s UserSystem) Delete(ids []uint) error {
	// 用户和角色存在多对多关联关系
...
func (s UserSystem) GetUserByIds(ids []uint) ([]model.User, error) {
	// 根据用户ID获取用户信息
	var userList []model.User
	err := common.DB.Where("id IN (?)", ids).Preload("Roles").Find(&userList).Error
	return userList, err
...
// ChangePwd 更新密码
func (s UserSystem) ChangePwd(username string, hashNewPasswd string) error {
...
// ChangeStatus 更新状态
func (s UserSystem) ChangeStatus(id, status int) error {
	return common.DB.Model(&model.User{}).Where("id = ?", id).Update("status", status).Error
}

// GetCurrentLoginUser 获取当前登录用户信息
// 需要缓存，减少数据库访问
func (s UserSystem) GetCurrentLoginUser(c *gin.Context) (model.User, error) {
...
// Login 登录
func (s UserSystem) Login(user *model.User) (*model.User, error) {
...
func (s UserSystem) ClearUserInfoCache() {
	userInfoCache.Flush()
}

// GetUserById 获取单个用户
func (us UserSystem) GetUserById(id uint) (model.User, error) {
	var user model.User
